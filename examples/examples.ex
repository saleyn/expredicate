defmodule Atree.Examples do
  ...
  def simple_example do
    ...
    # Create a new tree
    tree = case Atree.new() do
      {:ok, t} -> t
      _  -> raise "Failed to create tree"
    end
  ...
  def inventory_filtering_example do
    ...
    Enum.each(rules, fn {item_id, rule} ->
      {:ok, _} = Atree.insert(tree, item_id, rule)
    end)

    # Inventory items
    items = [
      {"laptop_1", %{"stock" => 5, "price" => 800, "discount_percentage" => 0, "monthly_sales" => 50, "profit_margin" => 35}},
      {"shirt_1", %{"stock" => 50, "price" => 25, "discount_percentage" => 30, "monthly_sales" => 500, "profit_margin" => 8}},
      {"phone_1", %{"stock" => 2, "price" => 600, "discount_percentage" => 50, "monthly_sales" => 200, "profit_margin" => 25}},
    ]
  ...
  def user_segmentation_example do
    ...
    # Example users
    users = [
      {"user_1000", %{"lifetime_spend" => 15000, "account_age" => 500, "days_since_login" => 2, "login_count" => 150, "status" => "active", "subscription_tier" => "premium"}},
      {"user_2000", %{"lifetime_spend" => 100, "account_age" => 10, "days_since_login" => 2, "login_count" => 5, "status" => "trial", "subscription_tier" => "trial"}},
      {"user_3000", %{"lifetime_spend" => 500, "account_age" => 400, "days_since_login" => 120, "login_count" => 10, "status" => "inactive", "subscription_tier" => "free"}},
    ]
  ...
  def benchmark_example do
    ...
    Enum.each(test_values, fn values ->
      {match_time, {:ok, matches}} =
        :timer.tc(fn ->
          Atree.match(tree, values)
        end)
  ...
  def operators_example do
    ...
    {:ok, matches} = Atree.match(tree, test_values)
